...
class LensOptimisation(Problem):
    def __init__(
        ...
        **kwargs,
        ...
        if training_instances is None:
            training_instances = [(seed,) for seed in range(1, 10)]
        if test_instances is None:
            test_instances = [(seed,) for seed in range(11, 16)]
        ...
        self.task_prompt = (
            ...
            if task_prompt is not None
                ...
                "FEEDBACK MECHANISM: You can optionally implement a `receive_feedback(self, info)` method in your Optimizer class. "
                "It will be called after each `func(x)` call with a dictionary containing {'loss': float, 'x_normalized': ndarray}.\n"
                "CONSOLE OUTPUT: Any print statements in your code will be captured and provided to you as feedback for the next generation.\n"
            )
        )
    ...
    def _build_objective(self):
        ...
        for _ in range(5):
            if (workspace_root / "camera-lens-simulation").exists():
                break
    ...
    def to_dict(self):
        return {"name": self.name, "budget_factor": self.budget_factor}
