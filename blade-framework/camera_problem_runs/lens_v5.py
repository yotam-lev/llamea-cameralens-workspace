...
def configure_run(llm, n_jobs):
    ...
    task_prompt = (
        ...
        "You are provided with `hess_func(x)`. When called with a full 24D vector, it returns an `(18, 18)` "
        "symmetric matrix representing the exact second derivatives of the 18 continuous parameters.\n"
        "- If manually stepping, you can compute a Newton step using `np.linalg.solve(H, -grad)`.\n"
        "- If using `scipy.optimize.minimize`, you MUST use a method that accepts an exact Hessian, "
        "such as `method='trust-exact'` or `method='Newton-CG'`, and pass it via the `hess=` argument.\n"
    ...
    mutation_prompts = [
    "Implement a Memetic Hybrid: Use Differential Evolution for global exploration. "
    "Every 5 generations, perform a local search on the 18 continuous variables "
    "using `scipy.optimize.minimize` with `method='trust-exact'`, utilizing both `grad_func` and `hess_func`.",
    
    "Second-Order Mutation Operator: Instead of random Gaussian noise, perturb the 18 continuous dimensions "
    "by taking a damped Newton step. Compute the gradient and Hessian, add a small regularization term to the Hessian diagonal (e.g., `H + 1e-4 * np.eye(18)`) to ensure positive-definiteness, and step using `np.linalg.solve(H, -grad)`.",
